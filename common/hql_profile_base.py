...
import re
import commands
from conf import hive as _base
...
def get_ast(hql,attach=''):
    ...
    if not os.path.exists(_base.TMP_SQL_PATH):
        os.mkdir(_base.TMP_SQL_PATH)

    now_time=time.strftime("%Y_%m_%d_%H_%M_%S",time.localtime(time.time()))
    random_num = random.randrange(0,10000)

    tmp_sql_file = '%s/tmp_%s_%s.sql' %(_base.TMP_SQL_PATH,now_time,random_num)
...
def profile_from_hql(hql, filename=None, type='hql',hql_type=1,attach=''):
   ...
   # ast_str = "(TOK_QUERY (TOK_FROM (TOK_TABREF (TOK_TABNAME dm dm_order_4analyst))) (TOK_INSERT (TOK_DESTINATION (TOK_DIR TOK_TMP_FILE)) (TOK_SELECT (TOK_SELEXPR (TOK_TABLE_OR_COL order_id)) (TOK_SELEXPR (TOK_TABLE_OR_COL user_id) uid) (TOK_SELEXPR (TOK_TABLE_OR_COL grouping__id)) (TOK_SELEXPR (TOK_FUNCTIONSTAR count) pv)) (TOK_WHERE (= (TOK_TABLE_OR_COL plat) 'mob')) (TOK_GROUPING_SETS (TOK_TABLE_OR_COL order_id) (TOK_TABLE_OR_COL user_id) (TOK_GROUPING_SETS_EXPRESSION (TOK_TABLE_OR_COL user_id)) (TOK_GROUPING_SETS_EXPRESSION (TOK_TABLE_OR_COL order_id)) (TOK_GROUPING_SETS_EXPRESSION (TOK_TABLE_OR_COL user_id) (TOK_TABLE_OR_COL order_id)) TOK_GROUPING_SETS_EXPRESSION)))"
    ...
    profile_msg = _profile_from_ast(ast_str, filename, 'struct')[0]
    return 0, profile_msg
...
def _profile_from_ast(ast_str, filename=None, format='neststr'):
    "format - struct: hive 0.11之后返回的语法树是结构化的换行表示; neststr: hive 0.11及之前，是括号结构的表示"

    assert ast_str is not None
    assert len(ast_str) != 0
    assert format in ('struct', 'neststr')
    ...
    if format == 'neststr':
        ...
        ast_ele_list = ast_str.strip().split('\n')
        ast_ele_list.reverse()
        ast_tree = _construct_ast_struct(ast_ele_list)
...
leading_blank_p = re.compile(r'^(\s*)')
def _construct_ast_struct(tree_str_list):
    line = tree_str_list.pop()
    current_leading_blank_count = len(leading_blank_p.match(line).group(1))
    line = line.strip()
    ...
    if line.startswith("TOK_"):
        ...
        while True:
            if not tree_str_list:
                break
            next_line = tree_str_list[-1] # just pre look
            next_blank_count = len(leading_blank_p.match(next_line).group(1))
            # child
            if next_blank_count > current_leading_blank_count:
                next_line = next_line.strip()
                if not next_line.startswith("TOK_"):
                    node.values.append(next_line)
                    consumed = tree_str_list.pop() # next line is consumed
                else:
                    node.children.append(_construct_ast_struct(tree_str_list))
            # not child
            else:
                break
        ...
        raise Exception("Unexcepted line:[{}] for a AST.".format(line))
