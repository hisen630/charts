# -*- coding: utf-8 -*-
from __base__ import TaskMapping
import json, time
...
class Manager(TaskMapping):
    ...
    #     if sqls:
    #         try:
    #             status,data = hive_base.get_profile(sqls)
    #             data = data.__dict__
    #         except Exception, e:
    ...
    def get_data_group(self, form, new_customs, timestramp):
        ...
        if sqls:
            try:
                ...
                data = hive_base.get_data_hive(sqls, profile, True)
            except Exception, e:
                return {'status': 0, 'msg': u'解析hql出错请检查是否正确'}
            if data['status']:
            ...
            if datas['status'] == 1:
                ...
                for item in datas['data']:
                    ...
                    result = AutoOper().write_data(task['id'], self.types, header, data, date,
                                                   table_tag=",".join(datas['data'][item]['dimensions']),
                                                   key=[[_time_column], datas['data'][item]['dimensions']])
            else:
                result = datas
        return result
