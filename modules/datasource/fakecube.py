...
class Manager(DataSource):
    ...
    def get_datasource_other(self,data):
        ...
        data['tasks'] = []
        for item in tasks:
            table_infos = task_b.get_relation_info(item['id'])
            tmp = []
            for it in table_infos:
                tmp_s = {}
                tmp_s['id'] = it['id']
                tmp_s['table_tag'] = it['table_tag']
                tmp.append(tmp_s)
            item['table_infos'] = json.dumps(tmp,default=defaultencode)
    ...
    def get_data(self,form,new_customs,rows='',istable=True,is_run_code=True):
        ...
        if tid and sqls:
            ...
            if check_result:
                ...
                data_result = AutoOper().read_data(tid,sqls,tid_table)
                try:
                    pass
                except Exception, e:
